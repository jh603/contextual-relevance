...
def main():
    ...
    plt.close(fig)
    print("nDCG summary saved to", out_dir / "delta_ndcg_summary.png")

    topics = BEIR_DATASETS
    policies = [
        "uniform",
        "ts_setrank_75_25",
        "ts_setrank_50_50",
    ...
    row = ["All datasets"]
    for policy in policies:
        # only keep seeds that ran on every dataset
        complete = [
            np.mean(vals)
